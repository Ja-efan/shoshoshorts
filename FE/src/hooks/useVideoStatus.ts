...
export function useVideoStatus(storyId: string) {
  const [videoStatus, setVideoStatus] = useState<VideoStatusUpdate | null>(null);
  ...
  // 처리 단계 텍스트 변환 함수
  const getProcessingStepText = useCallback((step: string | undefined): string => {
    if (!step) return "처리 중";
  ...
  const connect = useCallback(() => {
    ...
    try {
      apiService.getVideoStatusSSE(storyId)
        .then(response => {
          if (!response.ok) {
          ...
          const eventSource = new EventSource(`${response.url}`, { withCredentials: true });
          ...
          eventSource.onmessage = (event) => {
            try {
              ...
              if (data.status === "PROCESSING" && data.processingStep) {
                console.log(`[${storyId}] 처리 단계 업데이트: ${data.processingStep} (${getProcessingStepText(data.processingStep)})`);
              }
              ...
              if (data.status === 'COMPLETED' || data.status === 'FAILED') {
                console.log(`[${storyId}] 최종 상태 (${data.status}) 도달. SSE 연결 종료`);
          ...
          eventSource.onerror = (event) => {
            ...
            if (eventSourceRef.current) {
              ...
              console.log(`[${storyId}] 연결 재시도 ${retryCountRef.current}/${MAX_RETRY_COUNT}`);
      ...
      setError(`연결 오류: ${e instanceof Error ? e.message : '알 수 없는 오류'}`);
      setIsConnected(false);
    }
  }, [storyId, isConnected, getProcessingStepText]);
  
  // SSE 연결 종료 함수
  const disconnect = useCallback((notifyServer: boolean = false) => {
    console.log(`[${storyId}] SSE 연결 종료 요청. 서버 알림: ${notifyServer}`);
    ...
    // 서버에 연결 종료 알림 (필요한 경우)
    if (notifyServer) {
      apiService.closeVideoStatusSSE(storyId)
        .then(() => console.log(`[${storyId}] 서버에 연결 종료 알림 성공`))
        .catch(err => console.error(`[${storyId}] 서버에 연결 종료 알림 실패:`, err));
    }
  }, [storyId]);
