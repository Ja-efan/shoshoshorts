...
def encode_image_to_base64(image_path: str) -> Optional[str]:
    ...
    Returns:
        ...
        None: 파일이 존재하지 않거나 읽기 실패 시
    """
    # 파일 존재 확인
    if not image_path or not os.path.isfile(image_path):
        app_logger.error(f"이미지 파일이 존재하지 않습니다: {image_path}")
        return None
    ...
    try:
        ...
        app_logger.error(f"이미지 파일 인코딩 실패: {str(e)}, 경로: {image_path}")
        return None
...
def check_scene(scene: Scene) -> dict:
    ...
    if not scene.audios or len(scene.audios) == 0:
        ...
        for idx, audio in enumerate(scene.audios):
            if not audio.type:
                validation_errors[f"audio_{idx}_type"] = (
                    f"{idx+1}번째 오디오의 유형이 누락되었습니다."
                )
                result = False
            if not audio.text:
                validation_errors[f"audio_{idx}_text"] = (
                    f"{idx+1}번째 오디오의 텍스트가 누락되었습니다."
                )
                result = False

            # 대화 유형일 경우 character 필드 검증
            if audio.type == "dialogue" and not audio.character:
                validation_errors[f"audio_{idx}_character"] = (
                    f"{idx+1}번째 대화 오디오의 캐릭터 정보가 누락되었습니다."
                )
                result = False
