...
# 현재 사용 중인 환경 변수 (기본값은 RELEASE 환경)
...
def upload_file_to_s3(file_path: str, object_name: Optional[str] = None) -> dict:
    ...
    try:
        ...
        # 파일 업로드
        logger.info(f"S3에 파일 업로드 중: {file_path} -> {current_s3_bucket_name}/{object_name}")
        print(f"S3에 파일 업로드 중: {file_path} -> {current_s3_bucket_name}/{object_name}")
        s3_client.upload_file(file_path, current_s3_bucket_name, object_name)
