...
from app.core.api_config import openai_klingai_config, openai_stablediffusion_config
...
class OpenAIService:
    ...
    - 이미지 프롬프트 생성(KLING AI)
    - 이미지 프롬프트 생성(Stable Diffusion)
    """
    
    @staticmethod
    async def generate_image_prompt_klingai(scene: Scene, style: str="disney") -> str:
        """
...
"""
        ...
        try:
            ...
            response = client.chat.completions.create(
                model=openai_klingai_config.MODEL,
                messages=[
                    {"role": "system", "content": f"당신은 이미지 생성 프롬프트를 작성하는 전문가입니다. 주어진 장면을 '{style}' 스타일로 시각적으로 묘사하는 영어 프롬프트를 작성해주세요. {style_instruction}"},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=openai_klingai_config.MAX_TOKENS,
                temperature=openai_klingai_config.TEMPERATURE
            )
